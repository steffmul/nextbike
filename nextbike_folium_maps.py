...
def get_bearing(p1, p2):
    ...
    y = (np.cos(lat1) * np.sin(lat2) - (
        np.sin(lat1) * np.cos(lat2) * np.cos(long_diff))
        )
...
with open(
    r'C:\Users\steff\Documents\DataScience Bootcamp\Bike\stations.json') as f:
...
df_stations = pd.DataFrame(
    stations, columns=['uid', 'name', 'spot', 'lat', 'lng'])
...
top_pickup = df.groupby(
    ['from_station', 'from_lat', 'from_long', 'icon']
    ).size().sort_values(ascending=False).head(20)
top_pickup = top_pickup.to_frame()
top_pickup = top_pickup.reset_index()

top_return = df.groupby(
    ['to_station', 'to_lat', 'to_long', 'icon']
    ).size().sort_values(ascending=False).head(20)
    ...
    folium.Marker(
        ...
        icon=folium.features.CustomIcon(
            ...
            popup=(
                ...
                ' trips: ' + str(top_pickup.iloc[i, 4:5][0])
                    )
    ...
    folium.Marker(
        ...
        icon=folium.features.CustomIcon(
            ...
            popup=(
                ...
                ' trips: ' + str(top_return.iloc[i, 4:5][0]))
...
for i in range(0, len(top_station_combis)):
    folium.PolyLine(locations=([top_station_combis.iloc[i]['from_lat'],
                    ...
                    ' to '+ top_station_combis.iloc[i]['to_station']+
                    ' trips: '+str(top_station_combis.iloc[i]['cnt'])
                           )
                    ).add_to(stationmap)
stationmap
...
# Heatmap
stations = station_combis[
    ['from_lat', 'from_long']
    ][station_combis['from_station_mode'] == 'station']
floating = station_combis[
    ['from_lat', 'from_long']
    ][station_combis['from_station_mode'] == 'floating']
